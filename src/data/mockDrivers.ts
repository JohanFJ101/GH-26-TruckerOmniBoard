import { Driver } from '../types';

export const MOCK_DRIVERS: Driver[] = [
  {
    id: 'D-001',
    name: 'Marcus Webb',
    avatar: 'MW',
    truckId: 'T-4821',
    truckType: 'dry-van',
    status: 'driving',
    location: { lat: 41.8781, lng: -87.6298, city: 'Chicago', state: 'IL', lastUpdated: '2 min ago' },
    hos: { driveRemaining: 6.5, dutyRemaining: 9.0, cycleRemaining: 42.0, nextBreakIn: 2.5, restartAvailable: false },
    performance: { onTimeRate: 94, avgCostPerMile: 1.82, safetyScore: 91, totalMilesThisWeek: 1840 },
    currentLoadId: 'L-3301',
    phone: '(312) 555-0182',
  },
  {
    id: 'D-002',
    name: 'Sandra Okonkwo',
    avatar: 'SO',
    truckId: 'T-3390',
    truckType: 'reefer',
    status: 'on-duty',
    location: { lat: 35.1495, lng: -90.0490, city: 'Memphis', state: 'TN', lastUpdated: '5 min ago' },
    hos: { driveRemaining: 10.0, dutyRemaining: 13.5, cycleRemaining: 58.0, nextBreakIn: 4.0, restartAvailable: false },
    performance: { onTimeRate: 98, avgCostPerMile: 1.74, safetyScore: 97, totalMilesThisWeek: 2210 },
    currentLoadId: null,
    phone: '(901) 555-0241',
  },
  {
    id: 'D-003',
    name: 'Ray Delgado',
    avatar: 'RD',
    truckId: 'T-5501',
    truckType: 'dry-van',
    status: 'driving',
    location: { lat: 29.7604, lng: -95.3698, city: 'Houston', state: 'TX', lastUpdated: '1 min ago' },
    hos: { driveRemaining: 2.0, dutyRemaining: 4.5, cycleRemaining: 12.0, nextBreakIn: 2.0, restartAvailable: false },
    performance: { onTimeRate: 87, avgCostPerMile: 2.01, safetyScore: 78, totalMilesThisWeek: 3100 },
    currentLoadId: 'L-3298',
    phone: '(713) 555-0309',
  },
  {
    id: 'D-004',
    name: 'Tanya Rivers',
    avatar: 'TR',
    truckId: 'T-2204',
    truckType: 'flatbed',
    status: 'off-duty',
    location: { lat: 33.4484, lng: -112.0740, city: 'Phoenix', state: 'AZ', lastUpdated: '45 min ago' },
    hos: { driveRemaining: 11.0, dutyRemaining: 14.0, cycleRemaining: 65.0, nextBreakIn: 8.0, restartAvailable: true },
    performance: { onTimeRate: 96, avgCostPerMile: 1.79, safetyScore: 94, totalMilesThisWeek: 890 },
    currentLoadId: null,
    phone: '(602) 555-0178',
  },
  {
    id: 'D-005',
    name: 'James Patel',
    avatar: 'JP',
    truckId: 'T-6612',
    truckType: 'dry-van',
    status: 'sleeper',
    location: { lat: 36.1627, lng: -86.7816, city: 'Nashville', state: 'TN', lastUpdated: '3 hr ago' },
    hos: { driveRemaining: 11.0, dutyRemaining: 14.0, cycleRemaining: 55.0, nextBreakIn: 11.0, restartAvailable: false },
    performance: { onTimeRate: 91, avgCostPerMile: 1.88, safetyScore: 89, totalMilesThisWeek: 1650 },
    currentLoadId: null,
    phone: '(615) 555-0444',
  },
  {
    id: 'D-006',
    name: 'Debbie Kowalski',
    avatar: 'DK',
    truckId: 'T-1103',
    truckType: 'reefer',
    status: 'driving',
    location: { lat: 39.9526, lng: -75.1652, city: 'Philadelphia', state: 'PA', lastUpdated: '8 min ago' },
    hos: { driveRemaining: 7.5, dutyRemaining: 10.0, cycleRemaining: 38.0, nextBreakIn: 3.5, restartAvailable: false },
    performance: { onTimeRate: 93, avgCostPerMile: 1.91, safetyScore: 88, totalMilesThisWeek: 2050 },
    currentLoadId: 'L-3305',
    phone: '(215) 555-0622',
  },
  {
    id: 'D-007',
    name: 'Maria Garcia',
    avatar: 'MG',
    truckId: 'T-7219',
    truckType: 'dry-van',
    status: 'on-duty',
    location: { lat: 36.1699, lng: -115.1398, city: 'Las Vegas', state: 'NV', lastUpdated: '4 min ago' },
    hos: { driveRemaining: 10.5, dutyRemaining: 13.0, cycleRemaining: 60.0, nextBreakIn: 7.0, restartAvailable: false },
    performance: { onTimeRate: 95, avgCostPerMile: 1.77, safetyScore: 93, totalMilesThisWeek: 1920 },
    currentLoadId: null,
    phone: '(702) 555-0811',
  },
  {
    id: 'D-008',
    name: 'David Chen',
    avatar: 'DC',
    truckId: 'T-9004',
    truckType: 'dry-van',
    status: 'on-duty',
    location: { lat: 34.0522, lng: -118.2437, city: 'Los Angeles', state: 'CA', lastUpdated: '6 min ago' },
    hos: { driveRemaining: 9.0, dutyRemaining: 12.0, cycleRemaining: 48.0, nextBreakIn: 5.0, restartAvailable: false },
    performance: { onTimeRate: 92, avgCostPerMile: 1.85, safetyScore: 90, totalMilesThisWeek: 1750 },
    currentLoadId: null,
    phone: '(213) 555-0933',
  },
  {
    id: 'D-009',
    name: 'Sarah Johnson',
    avatar: 'SJ',
    truckId: 'T-5588',
    truckType: 'reefer',
    status: 'on-duty',
    location: { lat: 39.7392, lng: -104.9903, city: 'Denver', state: 'CO', lastUpdated: '3 min ago' },
    hos: { driveRemaining: 8.5, dutyRemaining: 11.5, cycleRemaining: 52.0, nextBreakIn: 6.0, restartAvailable: false },
    performance: { onTimeRate: 94, avgCostPerMile: 1.80, safetyScore: 92, totalMilesThisWeek: 2080 },
    currentLoadId: null,
    phone: '(303) 555-0477',
  },
];
